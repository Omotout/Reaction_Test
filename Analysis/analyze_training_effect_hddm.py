...
モデル:
  ...
  depends_on = {'v': ['stim', 'Phase'], 'a': 'Phase', 't': 'Phase'}
...
def plot_group_posteriors(deltas_a: dict, deltas_b: dict,
    ...
    for ax, param in zip(axes, ["v", "a", "t"]):
        ...
        ax.set_xlabel(f"Δ{param}{unit}   PostTest − Baseline")
        ax.set_ylabel("Posterior density")
        p_a = deltas_a[param]["p_reduction"]
        p_b = deltas_b[param]["p_reduction"]
        ax.set_title(f"Δ{param}    P(Δ<0): {name_a}={p_a:.3f}, {name_b}={p_b:.3f}")
